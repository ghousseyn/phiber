...
class main implements app
{
    ...
    function plugins ()
    {
        ...
        foreach ($this->bootstrap->plugins as $plugin) {
            $this->load($plugin, null, 
                    $this->config->library . "/plugins/" . $plugin . "/")->run();
        }
    }
    ...
    function checkSession ()
    {
        if ($this->config->inactive) {
            if (isset($_SESSION['user']['activity']) && (time() -
                     $_SESSION['user']['activity'] > $this->load('tools')->orDefault(
                            $this->config->inactive, 1800))) {
                ...
                if ($this->config->sessionReginerate) {
                    if (isset($_SESSION['user']['created']) && (time() -
                             $_SESSION['user']['created'] > $this->load('tools')->orDefault(
                                    $this->config->sessionReginerate, 1800))) {
                                session_regenerate_id(true);
                                $_SESSION['user']['created'] = time();
                            }
                        }
                        // $this->stack("Inactivity:
                    // ".($this->load('tools')->convertTime(time()
                    ...
                    function getView ()
                    {
                        ...
                        if ($path[0] == 'default') {
                            array_shift($path);
                            $path = $this->config->library . "/views/" .
                                     implode("/", $path) . ".php";
                                } else {
                                    $path = $this->config->library . "/modules/" .
                                             array_shift($path) . "/views/" . implode(
                                                    "/", $path) . ".php";
                                        }
                                        ...
                                        if ($this->get('layoutEnabled')) {
                                                    ...
                                                    $layout = null)
                                            {
                                                if (null != $layout) {
                                                    ...
                                                    include_once $this->config->library .
                                                             "/layouts/layout.php";
                                                        }
                                                    }
                                                    ...
                                                    function router ()
                                                    {
                                                        ...
                                                        if ($this->isValidURI(
                                                            ...
                                                            if (! empty(
                                                                    $parts[0]) && $this->bootstrap->isModule(
                                                                    $parts[0])) {
                                    ...
                                    function setVars ($parts)
                                    {
                                        ...
                                        foreach ($parts as $k => $val) {
                                            if ($k == 0 || ($k % 2) == 0) {
                                                // we dont't want to allow
                                                // spaces in
                                                // keys (as a good practice)
                                                $this->_requestVars[str_replace(
                                                        " ", "_", $parts[$k])] = $parts[$k +
                                                         1];
                                                    }
                                                }
